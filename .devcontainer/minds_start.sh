...
docker run --name mindsdb_container -p 47334:47334 -d mindsdb/mindsdb
...
if  docker ps --filter "name=mindsdb_container" --format '{{.Names}}' | grep -q mindsdb_container; then
